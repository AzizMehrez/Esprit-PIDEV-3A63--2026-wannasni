 ...
 * DOMContentLoaded fires only once; Turbo replaces body without re-firing it.
 * We listen to both events and guard against double-init.
 */
(function () {
    var _uploadInitDone = false;

    function initUploadPage() {
        if (_uploadInitDone) return;

        const dropZone = document.getElementById('dropZone');
        if (!dropZone) return; // Not on the upload page

        _uploadInitDone = true;
        console.log("Upload page initialized (Turbo-compatible)");
        ...
        uploadForm.addEventListener('submit', async function (e) {
            ...
            try {
                ...
                // Store meal calories for final total computation
                uploadForm.dataset.mealCals = data2.total_calories || 0;
                updateStep(2, 'done');
        ...
        function renderFinal(data) {
            ...
            try {
                // Compute total_day: prefer server value, fallback to consumed + meal calories
                const consumed = parseInt(uploadForm.dataset.consumed || 0);
                const mealCals = parseInt(uploadForm.dataset.mealCals || 0);
                const totalDay = Math.round(data.total_day || data.total_calories_day || (consumed + mealCals));

                const totalDayEl = document.getElementById('totalDayCals');
                if (totalDayEl) totalDayEl.innerText = totalDay;

                const limit = parseInt(uploadForm.dataset.limit || 2000);
                const percent = Math.min(100, Math.round((totalDay / limit) * 100));
                const bar = document.getElementById('calorieBar');
                if (bar) {
                    bar.style.width = percent + '%';
                    const barClass = percent >= 100 ? 'bg-danger' : percent >= 85 ? 'bg-warning' : 'bg-success';
                    bar.className = 'progress-bar ' + barClass;
                }

                // Also update the top calorie banner if present
                const bannerConsumed = document.querySelector('[data-calorie-consumed]');
                if (bannerConsumed) {
                    bannerConsumed.textContent = totalDay;
                }
                const bannerRemaining = document.querySelector('[data-calorie-remaining]');
                if (bannerRemaining) {
                    const rem = limit - totalDay;
                    bannerRemaining.textContent = rem >= 0 ? rem : '-' + Math.abs(rem);
                }
    ...
    // Run on Turbo navigations (fires after body replacement)
    document.addEventListener('turbo:load', function () {
        _uploadInitDone = false; // Reset guard for new page
        initUploadPage();
    });

    // Also handle turbo:render for cached pages
    document.addEventListener('turbo:render', function () {
        _uploadInitDone = false;
        initUploadPage();
    });
})();
