...
Réponds UNIQUEMENT en JSON avec ce format exact:
{
    ...
    private function analyzeWithSimulation(string $imagePath): array
    {
        ...
        if ($detectedType === null) {
            ...
            $confidence = 0.75 + (mt_rand(0, 15) / 100); // 0.75-0.90
        }
        ...
        switch ($detectedType) {
            case 'electricite':
                ...
                $urgency = mt_rand(0, 1) ? 'urgente' : 'moyenne';
                break;
            ...
            case 'plomberie':
                ...
                $description = $descriptions[array_rand($descriptions)];
                $urgency = mt_rand(0, 2) ? 'moyenne' : 'urgente';
                break;
            ...
            case 'transport':
                ...
                $description = $descriptions[array_rand($descriptions)];
                $urgency = mt_rand(0, 1) ? 'moyenne' : 'normale';
                break;
    ...
    private function detectTypeFromImageColors(array $analysis): string
    {
        ...
        if ($analysis['hasRed'] ?? false) {
            return \mt_rand(0, 1) ? 'electricite' : 'plomberie';
        }
