...
class UserHealthController extends AbstractController
{
    ...
    public function index(Request $request, HealthJournalRepository $repo): Response
    {
        ...
        $totalRecords = $repo->createQueryBuilder('h')
            ->select('count(h.id)')
        ...
        $healthRecords = $repo->createQueryBuilder('h')
            ->andWhere('h.senior = :senior')
            ->setParameter('senior', $this->getUser())
            ->orderBy('h.date', 'DESC')
    ...
    #[Route('/add', name: 'app_my_health_add', methods: ['GET', 'POST'])]
    public function add(Request $request, EntityManagerInterface $em): Response
    {
        ...
        // Add error messages if form was submitted but invalid
        if ($form->isSubmitted() && !$form->isValid()) {
