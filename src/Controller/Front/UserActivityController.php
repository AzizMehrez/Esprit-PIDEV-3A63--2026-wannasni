...
class UserActivityController extends AbstractController
{
    ...
    #[Route('/show/{id}', name: 'app_activity_show', requirements: ['id' => '\d+'])]
    public function show(int $id, Request $request): Response
    {
        ...
        if ($user instanceof User) {
            ...
            $isEnrolled = $participation && !in_array($participation->getStatus(), ['annulé', 'cancelled']);
        }

        // Fetch location data with coordinates
        $locationData = null;
        if ($activity->getLocation()) {
            $locationFile = $this->getParameter('kernel.project_dir') . '/public/data/locations.json';
            if (file_exists($locationFile)) {
                $locationsData = json_decode(file_get_contents($locationFile), true);
        ...
        $activityData = [
            ...
            'participants' => $this->participationRepository->countActiveByActivity($id),
            'maxParticipants' => $activity->getMaxParticipants(),
            'isFull' => $activity->isFull(),
