...
class NutritionAdminController extends AbstractController
{
    ...
    public function index(Request $request, RegimePrescritRepository $repository, EntityManagerInterface $em): Response
    {
        ...
        if ($searchTerm !== '') {
            // Search by type, user name, pathologies, or ID
            $qb = $repository->createQueryBuilder('r')
                ->leftJoin('r.user', 'u')
                ->where('r.typeRegime LIKE :search')
                ->orWhere('u.firstName LIKE :search')
                ->orWhere('u.lastName LIKE :search')
                ->orWhere('r.pathologies LIKE :search')
                ->orWhere('r.recommandationsSpeciales LIKE :search')
                ->orWhere('r.suiviRequis LIKE :search')
                ->setParameter('search', '%' . $searchTerm . '%')
                ->orderBy('r.dateDebut', $sort === 'asc' ? 'ASC' : 'DESC');

            // Also search by numeric ID if the term is numeric
            if (is_numeric($searchTerm)) {
                $qb->orWhere('r.id = :searchId')
                   ->orWhere('r.caloriesJournalieres = :searchId')
                   ->setParameter('searchId', (int) $searchTerm);
            }

            $regimePrescrits = $qb->getQuery()->getResult();
        } else {
