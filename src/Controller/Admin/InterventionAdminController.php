...
class InterventionAdminController extends AbstractController
{
    ...
    #[Route('/{id}/send-email', name: 'admin_interventions_send_email', requirements: ['id' => '\d+'], methods: ['POST'])]
    public function sendEmail(Intervention $intervention, Request $request, InterventionEmailService $emailService): Response
    {
        if (!$this->isCsrfTokenValid('send_email' . $intervention->getId(), $request->request->get('_token'))) {
            $this->addFlash('error', 'Token CSRF invalide.');
        ...
        try {
            ...
            $recipients = array_unique(array_filter([
                $service->getSeniorEmail(),
                $service->getUser()?->getEmail(),
            ]));
            $this->addFlash('success', '📧 Devis renvoyé avec succès à : ' . implode(', ', $recipients));
        } catch (\Throwable $e) {
            $this->addFlash('warning', '⚠️ Échec de l\'envoi : ' . $e->getMessage());
        }
