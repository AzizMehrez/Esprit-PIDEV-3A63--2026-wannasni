...
class ServiceAdminController extends AbstractController
{
    ...
    public function assign(
        ...
        if ($request->isMethod("POST")) {
            try {
                $validatorService->validateServiceAssignment($request->request->all());

                $technicienId = $request->request->get("technicien_id");
                // Technicien existence check is kept here as it requires DB access which validator might not have
