...
class ParticipationRepository extends ServiceEntityRepository
{
    ...
    /**
     * Find a participation by user and activity
     */
    public function findByUserAndActivity(int $userId, int $activityId): ?Participation
    {
        return $this->createQueryBuilder('p')
            ->andWhere('p.seniorId = :userId')
            ->andWhere('p.activityId = :activityId')
            ->andWhere('p.status IN (:statuses)')
            ->setParameter('userId', $userId)
            ->setParameter('activityId', $activityId)
            ->setParameter('statuses', ['registered', 'inscrit'])
            ->getQuery()
            ->getOneOrNullResult();
    }
    ...
    /**
     * Find recent participations (joins and cancellations)
     */
