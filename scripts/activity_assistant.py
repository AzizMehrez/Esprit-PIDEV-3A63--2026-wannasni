...
def detect_language(text):
    """Detect language from text — supports EN, FR, AR, Tunisian (Arabic-script and Latin/Franco)"""
    # Tunisian Latin/Franco keywords (check BEFORE Arabic script detection)
    tun_latin_words = ['chnowa', 'kifech', 'nheb', 'wqtesh', 'bahi', 'barcha', 'moch', '3andi',
                       'ya kho', 'roh l', 'b3id', 'hedha', 'famma', 'yezzi', 'chwaya',
                       'n7eb', 'w9tesh', 'taw', 'ahna', 'inscri-ni', 'beh t3addi']
    text_lower = text.lower()
    if any(w in text_lower for w in tun_latin_words):
        return 'tn'

    # Arabic script detection (could be Tunisian darija or MSA)
    if any('\u0600' <= char <= '\u06FF' for char in text):
        # Tunisian-specific Arabic words
        tn_arabic_markers = ['\u0634\u0646\u0648\u0627', '\u0643\u064a\u0641\u0627\u0634', '\u0646\u062d\u0628',
                             '\u0628\u0627\u0647\u064a', '\u0628\u0631\u0634\u0627', '\u0645\u0648\u0634',
                             '\u064a\u0632\u064a', '\u062e\u0630\u0646\u064a', '\u0631\u0648\u062d',
                             '\u0641\u0627\u0645\u0629', '\u0634\u0648\u064a\u0629', '\u062a\u0627\u0648',
                             '\u0647\u0630\u0627\u0643\u0627', '\u0627\u062f\u064a']
        if any(m in text for m in tn_arabic_markers):
            return 'tn'
        return 'ar'
...
def extract_intent_and_activity(user_text):
    ...
    navigate_keywords = ['go to', 'open', 'navigate', 'take me', 'aller', 'ouvrir', 'naviguer',
                         'emmène', 'amène', 'page', 'اذهب', 'افتح', 'انتقل']
    
    intent = 'chat'
    activity_name = ''
    
    # Check navigate intent first
    if has_keyword(text_norm, navigate_keywords):
        intent = 'navigate'
    elif has_keyword(text_norm, greeting_keywords) and len(text_norm.split()) <= 4:
        intent = 'greeting'
...
def get_smart_response(user_text, user_id, lang='en'):
    ...
    templates = {
        ...
        'tn': {
            'greeting': "أهلا! أنا هنا باش نساعدك. تقدر تطلب مني تعرض النشاطات، تسجل، ولا تشوف نشاطاتك 😊",
            'list_intro': "آها، هذي النشاطات المتاحة:",
            'no_activities': "مزيان، مافيش نشاطات الوقت هذا. حاول مرة أخرى!",
            'my_activities': "أنت مسجل في النشاطات هذي:",
            'no_my_activities': "مازلت ما سجلتش في شي نشاط. تحب نعرضلك اللي فامة?",
            'join_success': "باهي! سجلناك في {activity}. استمتع!",
            'join_already': "أنت مسجل كان في {activity}!",
            'no_activity_match': "ما لقيتش النشاط هذا. حاول تقول اسمو مرة أخرى?",
            'cancel_success': "الغينا تسجيلك في {activity}. مشكلة!",
            'cancel_notfound': "ما سجلتش في النشاط هذا.",
            'help': "نقدر نساعدك! جرب: 'عرضلي النشاطات'، 'سجلني في اليوغا'، 'شنو نشاطاتي?'، ولا 'الغيلي السباحة'"
        }
    }
    ...
    try:
        if intent == 'greeting':
            ...
            # Navigation handled by voice API; here just return a prompt message
            nav_msgs = {
                'en': "Sure! Tell me where you want to go: dashboard, profile, activities, health, nutrition, services, or messages.",
                'fr': "Bien sûr! Dites-moi où vous voulez aller: tableau de bord, profil, activités, santé, nutrition, services ou messages.",
                'ar': "حسناً! قل لي أين تريد الذهاب: لوحة التحكم، الملف الشخصي، الأنشطة، الصحة، التغذية، الخدمات أو الرسائل.",
                'tn': "باهي! قلي وين تحب تمشي: الداشبورد، البروفيل، النشاطات، الصحة، التغذية، الخدمات ولا الرسائل.",
            }
            response_text = nav_msgs.get(lang, nav_msgs['en'])
            display_text = response_text
