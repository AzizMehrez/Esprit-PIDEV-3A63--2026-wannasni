...
Be conversational, helpful, and use emojis! Format code with proper syntax highlighting. Address the user by name when possible. 👋`;
    ...
    async tryModelsWithFallback() {
        for (let i = 0; i < this.freeModels.length; i++) {
            try {
                this.model = this.freeModels[i];
                console.log(`🔄 Trying model ${i + 1}/${this.freeModels.length}: ${this.model}`);
                ...
                console.warn(`❌ Model ${this.model} failed:`, error.message);
                
                if (i === this.freeModels.length - 1) {
                    // All API models failed - use intelligent fallback
                    console.log("🤖 All AI models failed, using intelligent fallback system");
